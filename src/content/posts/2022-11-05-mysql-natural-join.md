...
natural/using 在 select _ 时，输出的列信息与 join on 有一点小小的区别， natural/using 都是按照 sql2003 标准进行 select _ 补全的，具体如下：
...
Answer：在 field 迭代器的代码中可以看到，只要一个 TABLE*LIST 的 is_join_columns_complete 置为 true 以后，field 迭代的内容就是 join_columns。而在扩展 select * 的关键函数 `setup_wild` 中可以看到，扩展 select \_ 时就是使用了 field 迭代器，这便用上了 join_columns。
